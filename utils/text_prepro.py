...
def load_cnn_data(file_path):
    cnn_data = []
    with open(file_path, "r") as f:
        for line in f:
            cnn_data.append(json.loads(line))

    random.shuffle(cnn_data)
    dev_sample_index = -1 * int(0.1 * float(len(cnn_data)))
    train_text, dev_text = cnn_data[:dev_sample_index], cnn_data[dev_sample_index:]

    return train_text, dev_text

def load_nmt_data(source, target, file_path):
    print("Reading lines...")
    # 파일 읽고 줄 단위로 나누기
    dataset_identi = file_path.split("/")[-1]
    source_text = list(open(file_path + "/" + dataset_identi + "." + source, "r", encoding='UTF-8').readlines())
    source_text = [clean_str(sent, True) for sent in source_text]
    target_text = list(open(file_path + "/" + dataset_identi + "." + target, "r", encoding='UTF-8').readlines())
    target_text = [clean_str(sent, True) for sent in target_text]

    print("Read %s source sentence" % len(source_text))
    print("Read %s target sentence" % len(target_text))
    print(source_text[0])
    print(target_text[0])
